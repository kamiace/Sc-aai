...
object SkillStore {
    ...
    suspend fun save(ctx: Context, skills: List<SkillCapsule>) {
        val normalized = skills.distinctBy(::stableKey)
    ...
    suspend fun mergeRemote(ctx: Context, remote: List<SkillCapsule>): List<SkillCapsule> {
        val merged = linkedMapOf<String, SkillCapsule>()
        load(ctx).forEach { merged[stableKey(it)] = it }
        remote.forEach { incoming ->
            val key = stableKey(incoming)
            val current = merged[key]
            if (current == null || incoming.version >= current.version) merged[key] = incoming
        }
    ...
    suspend fun delete(ctx: Context, id: String) = save(ctx, load(ctx).filterNot { it.id == id })
