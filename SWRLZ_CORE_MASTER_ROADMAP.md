...
**Roadmap revision:** 1.2 — Pzhp-LDS integrated  
...
# 0.1.6 — Pzhp-LDS Local Core, Patch Manifests, and Learning Delta Sync

- [ ] **LDS-001 — Local-first node prototype.** 🧩 **Seeded in the Pzhp-LDS source; build and runtime testing still required before checking it off.** A lightweight local server can expose `/status`, `/command`, `/teach`, `/logs`, `/sync`, and `/update-check` without paid AI dependencies.
- [ ] **LDS-002 — Minimum local core file set.** 🧩 **Seeded in the Pzhp-LDS source; integration testing still required before checking it off.** Adds `version.json`, `patch_manifest.json`, `rules.json`, `skills.json`, `device_profile.example.json`, append-only delta/log examples, merge report example, rollback folder, and JSON schema.
- [ ] **LDS-003 — Learning delta packet schema.** 🧩 **Seeded in the Pzhp-LDS source; Android Teach Mode integration still required.** Defines how a user-taught skill records device, time, core version, trigger phrases, steps, safety level, scope, approval state, and sync state.
- [ ] **LDS-004 — Append-only local deltas.** 🧩 **Prototype exists in `backend/local_node_server.py`; Android integration remains planned.** New teachings are appended to `local_deltas.jsonl` instead of replacing previous local learning.
- [ ] **LDS-005 — Patch manifest compare-only check.** 🧩 **Prototype exists; real remote manifest retrieval and hash verification remain planned.** The node can compare local and manifest versions without applying risky changes.
- [ ] **LDS-006 — Backup → compare → download → verify → stage → merge → test → activate → log flow.** ⏳ **Planned.** The docs define the safe patch lifecycle; the implementation still needs download, hashing, staging, merge, activation, and rollback behavior.
- [ ] **LDS-007 — Merge engine rules.** ⏳ **Planned.** Conflicts should become review items, stricter safety should win by default, and local deltas must never be erased without explicit approval.
- [ ] **LDS-008 — Merge report generation.** 🧩 **Example report exists; live generation still planned.** Every update/sync should produce a human-readable report showing what changed, what was kept, what conflicted, and what was rolled back.
- [ ] **LDS-009 — Sync scopes.** ⏳ **Planned.** Skills may remain private-local, personal-synced across the user's devices, or promoted as official candidates after review.
- [ ] **LDS-010 — Native Android/Kotlin service path.** ⏳ **Planned.** The Python prototype proves the flow first; later the local node should become a proper Android foreground service or native local server.


# 0.1.7 — Mission Ledger, Recovery, and Local Skills
...
# 0.1.9 — Multi-Mentor and Shared Swurlz Profile
...
# 0.2.0 — Continuity Reconciliation
