...
import { API_TOKEN, BACKEND_CONFIGURED, WS_BASE, CONSOLE_SESSION_KEY } from "@/constants/api";
...
export default function Cockpit() {
  ...
  useEffect(() => {
    if (!BACKEND_CONFIGURED) {
      setPlanRationale("Offline mode: configure a Mentor backend for Council chat and live planning. Local skills remain available.");
      return;
    }
    jsonGet("/missions").then(setMissions).catch((error) => setPlanRationale(error.message));
  ...
  useEffect(() => {
    if (!active || !BACKEND_CONFIGURED || !WS_BASE) return;
    ...
    const url = `${WS_BASE}/missions/${active}/stream${token}`;
  ...
  const startMission = async () => {
    if (!goal.trim()) return;
    if (!BACKEND_CONFIGURED) {
      setPlanRationale("Online Mentor is not configured. This mission was not started; local skills remain available.");
      return;
    }
    try {
      ...
      setPlanRationale(error.message);
    }
  };
  ...
  return (
    <div className="space-y-3">
      {!BACKEND_CONFIGURED && (
        <div className="border border-[#EAB308]/50 p-3 font-mono text-xs text-[#EAB308]">
          OFFLINE MODE · Council chat and new planning are disabled until a Mentor backend is configured. Local Skill Library remains active.
        </div>
      )}
