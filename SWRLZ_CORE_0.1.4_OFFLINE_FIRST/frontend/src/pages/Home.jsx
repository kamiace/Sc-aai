...
export default function Home() {
  const apkAvailable = (process.env.REACT_APP_APK_AVAILABLE || "false").toLowerCase() === "true";
  return (
      ...
      <section className="relative overflow-hidden border border-[#1F1F22] bg-[#0A0A0B] scanlines">
        ...
        <div className="relative p-10 md:p-16">
          ...
          <div className="mt-10 flex flex-wrap gap-3">
            ...
            {apkAvailable ? (
              <a
                href="/apk/swurlz-core-debug.apk"
                download
                data-testid="hero-download-apk"
                className="font-mono text-xs uppercase tracking-[0.3em] px-5 py-3 border border-[#1F1F22] text-[#8A8A8E] hover:text-[#E3E3E2]"
              >
                ↓ Download APK · offline-first
              </a>
            ) : (
              <span className="font-mono text-xs uppercase tracking-[0.3em] px-5 py-3 border border-[#1F1F22] text-[#5A5A5E]">
                APK build not bundled
              </span>
            )}
          </div>
        </div>
