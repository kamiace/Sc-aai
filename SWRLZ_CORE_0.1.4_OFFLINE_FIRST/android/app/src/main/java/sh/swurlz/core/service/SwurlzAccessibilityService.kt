...
class SwurlzAccessibilityService : AccessibilityService() {
    ...
    fun snapshot(maxNodes: Int = 80): Snapshot {
        nodeIndex.clear()
        val root = rootInActiveWindow ?: return Snapshot(emptyList(), null, "no root window")
        ...
        return Snapshot(nodes, root.packageName?.toString(), summary)
    }

    data class Snapshot(val nodes: List<UiNode>, val pkg: String?, val summary: String)
